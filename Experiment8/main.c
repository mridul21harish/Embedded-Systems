...
void printChar(char c, int uart, int flag);
char *readString(char delimiter, int uart, int flag);
void printString(char * string, int uart, int flag);
...
int main()
{
  ...
  // Configure the GPIO current level and/or slew rate as specified for the mode selected  (see page 673 and page 681
  // Configure the PMCn fields in the GPIOPCTL register to assign the UART signals to the appropriate pins (see page 688 and Table 23-5 on page 1351).
  GPIO_PORTA_PCTL_R = (1 << 0) | (1 << 4);

  GPIO_PORTA_DEN_R = (1 << 0) | (1 << 1);

  // Find the Baud-Rate Divisor
  // BRD = 16,000,000 / (16 * 9600) = 104.1666666666666666666666666666
